...
class AmazonReviewsSpider(scrapy.Spider):
    ...
    def __init__(self, url, *args, **kwargs):
        ...
        if re.match('^https\:\/\/www\.amazon\.com\/?.*\/dp\/[a-zA-Z0-9]*\/?.*$', url) is not None:
            results = re.search('^https\:\/\/www\.amazon\.com\/?.*\/dp\/([a-zA-Z0-9]*)\/?.*$', url)
            product_id = results.group(1)
            url = 'https://www.amazon.com/product-title/product-reviews/' + product_id + '/ref=cm_cr_arp_d_viewopt_srt?ie=UTF8&reviewerType=all_reviews&sortBy=recent&pageNumber=1'
        else:
            driver.close()
    ...
    def parse_review(self, review_div):
        ...
        try:
            ...
            amazon_item['user_name'] = review_div.xpath(".//div[@class = 'a-profile-content']/span/text()").extract()[0].strip()
            amazon_item['review_title'] = review_div.xpath(".//div[@class = 'a-row']").xpath(
                ".//a[contains(@class, 'review-title-content')]/span/text()").extract()[0].strip()
            ...
            amazon_item['review_data'] = ' '.join(
                review_div.xpath(".//span[@class='a-size-base review-text review-text-content']/span/text()").extract()).strip()
            return amazon_item
        except:
